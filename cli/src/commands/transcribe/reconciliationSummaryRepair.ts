...
const forbidden =
  /(?:secret|token|password|credential|authorization|bearer|transcript|(?:\/[A-Za-z0-9_.-]+){2,})/iu;
...
function safePath(path: readonly PropertyKey[]): string[] {
  return path
    .slice(0, MAX_DEPTH)
    .map((part) => String(part).slice(0, MAX_SEGMENT));
}
function safeMessage(message: string): string {
  return (
    forbidden.test(message)
      ? "validation feedback omitted for privacy"
      : message
  ).slice(0, MAX_MESSAGE);
}
export function normalizeSummaryRepairIssues(
  error: unknown,
): SummaryRepairIssue[] {
  ...
  if (error instanceof z.ZodError)
    return error.issues.slice(0, MAX_ISSUES).map((issue) => ({
      ...
      ...(issue.code === "invalid_value"
        ? { allowed: (issue as { values?: string[] }).values?.slice(0, 128) }
        : {}),
    }));
  const message =
    error instanceof Error ? error.message : "authoritative validation failed";
  return [
    {
      stage: "semantic",
      code: "authoritative-validation",
      path: [],
      message: safeMessage(message),
    },
  ];
}
function boundedOriginal(value: unknown): string {
  let text: string;
  try {
    text =
...
export function buildSummaryRepairPrompt(
  ...
  const issues = context.issues.slice(0, MAX_ISSUES).map((issue) => ({
    stage: issue.stage,
    code: issue.code,
    path: issue.path.slice(0, MAX_DEPTH).map((p) => p.slice(0, MAX_SEGMENT)),
    message: issue.message.slice(0, MAX_MESSAGE),
    ...(issue.allowed
      ? {
          allowed: issue.allowed
            .slice(0, 128)
            .map((value) => value.slice(0, MAX_SEGMENT)),
        }
      : {}),
  }));
  const prefix = [
  ...
  if (
    ...
    SUMMARY_REPAIR_BOUNDS.maxPromptBytes
  ) {
  ...
  if (originalBytes > Math.min(MAX_ORIGINAL_BYTES, available))
    throw new Error("summary repair original response exceeds bound");
  return `${prefix}${original}${suffix}`;
}
export const SUMMARY_REPAIR_BOUNDS = {
  maxIssues: MAX_ISSUES,
  maxPathDepth: MAX_DEPTH,
  maxPathSegment: MAX_SEGMENT,
