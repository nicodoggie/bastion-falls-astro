...
export function buildUnifiedReconciliationPrompt(job: ReconciliationChunkJob): string {
  const { packet } = job;
  return ["You are performing read-only transcript reconciliation.", `Owned logical window: ${packet.chunk.id} ${packet.chunk.start}-${packet.chunk.end}.`, "Produce a readable transcript, not a narrative digest. Preserve recoverable dialogue and narration, turn structure, meaningful repetition, interruption, uncertainty, and code-switching. Merge events only when they represent the same utterance; omit only content matching an allowed omission reason. A channel-only or isolated candidate is not automatically speech: when the supplied evidence does not establish that a candidate was spoken, preserve its exact source event as an unclear review-only block (with unknown attribution and an uncertainty review flag) rather than presenting it as dialogue; do not use a short token, channel, or energy alone to infer speech.", "Only authoritativeSourceEvents in this packet may be consumed, omitted, or emitted.", "Previous readable text and next alignment events are context-only; neighboring events are forbidden.", "Use unknown attribution rather than inventing a character. Summary-safe text must correspond block-for-block.", RECONCILIATION_OUTPUT_CONTRACT, "All required schema and ownership evidence follows. Do not search the repository for schemas or output examples. Bounded read-only repository retrieval is allowed only to verify a specific uncertain proper noun or lore claim; it cannot change evidence ownership or create support absent from the supplied packet.", JSON.stringify({ packet, authoritativeSourceEvents: job.authoritativeSourceEvents }, null, 2)].join("\n");
}
