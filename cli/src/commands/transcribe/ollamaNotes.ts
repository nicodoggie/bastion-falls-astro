...
export function splitTextByLines(text: string, maxChars: number): string[] {
  ...
  for (const line of text.split("\n")) {
    const additionalLength =
      current.length === 0 ? line.length : line.length + 1;
...
export async function runOllamaHierarchicalNotes(
  options: OllamaNotesOptions,
): Promise<void> {
  const transcript = await readFile(options.transcriptPath, "utf8");
  const correctionNotes =
    options.correctionNotesPath && (await exists(options.correctionNotesPath))
  ...
  for (const [index, chunk] of transcriptChunks.entries()) {
    ...
    const reused = options.resume && !options.force && (await exists(path));
  ...
  for (
    ...
    index += options.sceneGroupSize
  ) {
    const groupIndex = index / options.sceneGroupSize;
    const group = chunkSummaries
      .slice(index, index + options.sceneGroupSize)
      .join("\n\n---\n\n");
    const path = join(
      sceneDir,
    ...
    const sceneTotal = Math.ceil(
      chunkSummaries.length / options.sceneGroupSize,
    );
    const reused = options.resume && !options.force && (await exists(path));
    await options.onWorkUnit?.({
      operation: "Summarize scene",
      workUnit: { label: "scene", index: groupIndex, total: sceneTotal },
      status: reused ? "reused" : "started",
    });
    await writeGeneratedFile({
      path,
  ...
  const reusedFinal =
    options.resume && !options.force && (await exists(finalDraftPath));
  await options.onWorkUnit?.({
    operation: "Write final summary",
    workUnit: { label: "final summary", index: 0, total: 1 },
  ...
  await writeFile(
    options.notesPath,
    bindNotesFrontmatter(mdx, options),
    "utf8",
  );
  await options.onWorkUnit?.({
    operation: "Write final summary",
