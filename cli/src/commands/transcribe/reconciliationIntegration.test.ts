...
test("stage allows a full twenty minutes for long reconciliation chunks", async () => {
  ...
  const onRetry = () => {};
  const stage = await runUnifiedReconciliationStage({ ...base("three"), rootDir, onRetry }, { runUnifiedReconciliation: async ({ jobs, timeoutMs, onRetry: observedRetry }) => { assert.equal(observedRetry, onRetry); calls += jobs.length; observedTimeout = timeoutMs; return { chunks: jobs.map((job) => ({ chunk: job.packet.chunk, schemaVersion: job.packet.schemaVersion, promptVersion: job.packet.promptVersion, cacheIdentity: job.packet.cacheIdentity, blocks: [{ id: "b", start: job.packet.ownedEvents[0]!.start, end: job.packet.ownedEvents[0]!.end, kind: "dialogue", text: "x", summarySafeText: "x", characterConfidence: "unknown", attributionBasis: ["source"], sourceEventIds: [job.packet.ownedEvents[0]!.id], reviewFlags: [] }], omissions: [], materialCorrections: [], suspicionFlags: [], reviewNotes: [], summarySafety: { status: "valid", errors: [] }, status: "needs_review" as const } as any)), repairedChunkIds: [], reusedChunkIds: [], diagnosticsDir: join(rootDir, "diagnostics") }; } });
...
test("structured notes calls injected summarizer and renderer, then writes MDX", async () => {
  ...
  const result = await runUnifiedStructuredNotes({ outputRoot: rootDir, chunks: [chunk], jobs: [job] }, { summarizer: async (options) => { summaryCalls++; assert.equal(options.timeoutMs, 900_000); return { chunks: [], scenes: [], session: {} as any }; }, renderer: () => "---\ntitle: Test\n---\n", writer: async (path, content) => writeFile(path, content) });
