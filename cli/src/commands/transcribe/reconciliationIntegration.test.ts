...
test("builds single, per-chunk, and deterministic three windows", () => {
...
test("stage invokes injected runner once and reports needs_review without blocking", async () => {
  ...
  assert.equal(calls, 3); assert.equal(stage.status, "needs_review"); assert.equal(stage.metadata.completedChunkIds.length, 3); await rm(rootDir, { recursive: true, force: true });
});
