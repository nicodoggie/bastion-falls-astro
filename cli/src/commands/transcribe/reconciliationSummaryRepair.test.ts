...
test("contracts and repair version are deterministic and complete", () => {
  ...
  for (const [level, fields] of Object.entries({
    chunk: [
      "summary.chunk.v1",
      "reconciliationBlockIds",
      "blockIds",
      "hooks",
      "sourceReviewDispositions",
    ],
    scene: ["summary.scene.v1", "chunkClaimProvenance", "chunkHookIds"],
    session: ["summary.session.v1", "provenanceMap", "promptVersion"],
  })) {
...
test("repair feedback is bounded, normalized, and private-safe", () => {
  const error = new z.ZodError([
    {
      code: "invalid_value",
      path: ["claims", "x".repeat(500)],
      message: "secret /home/ensu/private transcript",
    } as never,
  ]);
  ...
  const prompt = buildSummaryRepairPrompt({
    ...
    originalResponse: "secret /home/ensu/private transcript",
    issues,
    authoritativeDomains: ["b0"],
  });
...
test("stable categories distinguish repairable validation from operational failures", () => {
  ...
  assert.equal(
    classifySummaryRepair(new SyntaxError("bad json")).eligible,
    true,
  );
  assert.equal(
    classifySummaryRepair(
      Object.assign(new Error("timeout"), { repairCategory: "timeout" }),
    ).eligible,
    false,
  );
  assert.equal(
    classifySummaryRepair(
      Object.assign(new Error("empty"), { repairCategory: "empty-output" }),
    ).eligible,
    false,
  );
  assert.equal(
    classifySummaryRepair(new Error("timeout: provider text")).eligible,
    false,
    "untyped text must fail closed",
  );
  assert.equal(
    classifySummaryRepair(
      Object.assign(new Error("anything"), {
        repairCategory: "semantic-validation",
      }),
    ).eligible,
    true,
  );
...
test("oversized non-original repair framing fails closed", () => {
  assert.throws(
    () =>
      buildSummaryRepairPrompt({
        level: "chunk",
        contract: "x".repeat(2_000_001),
        originalResponse: "{}",
        issues: [],
      }),
    /framing exceeds bound/iu,
  );
  const multibyte = buildSummaryRepairPrompt({
    level: "chunk",
    contract: "contract",
    originalResponse: "😀".repeat(400_000),
    issues: [],
  });
...
test("complete repair prompt rejects an oversized original instead of truncating it", () => {
  assert.throws(
    () =>
      buildSummaryRepairPrompt({
        level: "chunk",
        contract: "contract",
        originalResponse: { extra: "x".repeat(2_000_000) },
        issues: [],
      }),
    /original response exceeds bound/iu,
  );
  assert.doesNotThrow(() =>
    buildSummaryRepairPrompt({
      level: "chunk",
      contract: "contract",
      originalResponse: { extra: "x".repeat(1_990_000) },
      issues: [],
    }),
  );
});
