...
`Age` value from cumulative existence time when no authored override exists, preserve authored
`age` as an explicit override, and let readers expand the Age row into a compact mortality history.
...
remains prose canon for now, and her authored `age: 16` remains an override rather than forcing the
...
`character.details.age` remains an optional authored override. When present, it is the displayed Age
even if the mortality phases could derive another value. Phase durations are still resolved for the
expanded history and audit evidence.
...
1. return authored `details.age` when present;
1. otherwise return a complete valid phase-derived Age when available;
1. otherwise omit Age.

This preserves Fawn's uncertain but authored age and Minfilia's current body-oriented age while
letting characters without overrides remain live and calendar-derived. An override changes only the
displayed total; it does not rewrite phase dates or derived phase durations to force agreement.
...
The unknown resurrection date remains absent. His current Age therefore uses an authored override
when present and is otherwise omitted until enough phase evidence is recovered.
...
- report matching and conflicting authored overrides when complete derivation exists;
...
The audit must not treat an intentionally authored override as invalid. When phase history is
complete, the audit reports agreement or disagreement as evidence without changing display
precedence.
...
1. Focused resolver tests cover:
   ...
   - authored override precedence with complete, incomplete, and invalid phase derivation;
...
1. Astro component/build checks verify:
   ...
   - authored override characters render the override while preserving honest phase history.
