...
The `creatureStats` frontmatter field on species pages lets you embed one or
more D&D 5e stat blocks directly in an article. Each key is an arbitrary
identifier you choose; the value is a creature object following the
...
The smallest valid stat block needs a name, size, type, AC, HP, speed, the six
ability scores, and a CR.
...
Add `canHover: true` alongside `fly` for hovering creatures. Omit any speed type
the creature lacks.
...
`number` — The raw score (not the modifier). Modifiers and saving throw bonuses
...
`object` — Saving throw bonuses for proficient saves. Only include the saves
where the creature has proficiency (or expertise). Values are formatted strings
including the sign:
...
`object` — Skill bonuses. Same format as `save`: only list skills where the
creature has proficiency or expertise:
...
Damage resistances, immunities, and vulnerabilities. Accepts an array of damage
type strings or condition objects:
...
`array` — Passive features. Each entry needs a `name` and `entries` (a list of
description strings):
...
**Recharge mechanic** — there is no separate `recharge` field in the
schema. Embed the recharge note directly in the action `name`, exactly
  ...
  - name: "Fire Breath (Recharge 5–6)"
    entries:
      - >
        ...
        area must make a DC 18 Dexterity saving throw, taking 56 (16d6)
        fire damage on a failed save, or half on a success.
...
`array` — Legendary actions. Same shape as `action`. Add `legendaryHeader` for
