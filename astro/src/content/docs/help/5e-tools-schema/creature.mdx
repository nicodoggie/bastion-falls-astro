...
The `creatureStats` frontmatter field on species pages lets you embed one or more D&D 5e stat blocks
directly in an article. Each key is an arbitrary identifier you choose; the value is a creature
object following the [5etools bestiary schema](https://github.com/TheGiddyLimit/5etools-utils).
...
The smallest valid stat block needs a name, size, type, AC, HP, speed, the six ability scores, and a
CR.
...
Common types: `aberration`, `beast`, `celestial`, `construct`, `dragon`, `elemental`, `fey`,
...
`number` — The raw score (not the modifier). Modifiers and saving throw bonuses are computed
automatically.
...
`object` — Saving throw bonuses for proficient saves. Only include the saves where the creature has
proficiency (or expertise). Values are formatted strings including the sign:
...
`object` — Skill bonuses. Same format as `save`: only list skills where the creature has proficiency
or expertise:
...
Damage resistances, immunities, and vulnerabilities. Accepts an array of damage type strings or
condition objects:
...
**Recharge mechanic** — there is no separate `recharge` field in the schema. Embed the recharge note
directly in the action `name`, exactly as it appears in print:
...
`array` — Legendary actions. Same shape as `action`. Add `legendaryHeader` for the introductory
paragraph:
