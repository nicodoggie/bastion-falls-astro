...
The page title comes from Starlight's `title` field;
...
`VehicleStatBlock` is registered
...
A usable ship block needs hull AC/HP, the six object ability scores,
and enough identity fields for your layout.
Pace and capacities match the Ghosts of Saltmarsh style tables.
...
`string` — Display name in the card header.
...
`"SHIP"` — Literal constant in 5etools data.
...
`integer` — The vehicle object's ability scores (not modifiers).
...
`array` — Steering, helm, bridge, etc.
Each item:
...
`unknown[]` — Ship-wide actions (strings or entry objects).
Optional.
...
Fields like `source`, `page`, `token`, `fluff`,
...
`@bastion-falls/5e-schema-zod` also exports Zod schemas
