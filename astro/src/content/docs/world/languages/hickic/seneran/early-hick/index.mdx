...
[Surrey-on-the-Brams](/world/locations/surrey-on-the-brams/). Details in
this is a modern analysis of that sketch using modern linguistic methods.

Those who spoke Hick, did not call it such, as the term *Hick* was exonymic, and
was given by the Kalassarians. The people of Senera called their language
*takutakal*, which translates to "the people's rhythm". Despite the potential
...
A reconstruction of a hypothetical [Proto-Hick](/world/languages/hickic/proto-hick/)
...
with the *absolutive* case marker -es. The "subject" or "agent" referent is
marked with the *ergative* case marker -el.
...
In the above example, the handler *materok* is the agent of the verb "to carry"
and is marked with the ergative case marker -el. The person *'al* is the patient
...
The single argument of the intransitive verb *barak'er* is the one performing
...
concepts and are marked with the verbalizer affix *-'er*.
...
The above example shows the word *ven* that means yoke or burden with the
verbalizer *-er* affixed to it. It then acts as the verb of the sentence that
means "to carry".

While some words, such as *barak* "walk", semantically denote actions, the
verbalizer affix *-'er* is still applied in most instances.
...
The above example shows the man *gal* entering the boat *rismater*. There is
another way to express this sentence using the spatial case marker, removing the
explicit verbal word *bel* "entrance".
...
may be appended to *rismater* to indicate that the man entered the boat.
...
surface, the prefix *ka-* was initially used to denote similarity. Over time, it
...
* thral-'er "to be sacred" (See:
  [Sacred Terms](/world/languages/hickic/seneran/early-hick/lexicon-by-field#sacred))
* mar-'er "to be red" (See: [Colors](/world/languages/hickic/seneran/early-hick/lexicon-by-field#colors))
...
obligation. See also: [Sacred Terms](/world/languages/hickic/seneran/early-hick/lexicon-by-field#sacred)
...
that had occurred in the distant past. The marker *branrum* is often only ever
...
It instead marks on an action word and replaces the verbalizer *-'er*. It has
