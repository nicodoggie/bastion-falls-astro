---
title: Elara Navton
tags:
  - characters
  - Tidemark Local Government
  - Tidemark Harbor Authority
character:
  name: Elara Navton
  details:
    age: 45
    dateOfBirth: 1230-03-12 AI
    sex: Female
    pronouns: She/Her
    mortality: alive
    species: human
    sexOrgans: []
  relationships:
    organizations: []
    relatives: []
    religions: []
    families: []
---

_Elara Navton_ is the current Harbor Master of [Tidemark](../locations/tidemark.mdx), overseeing
port operations and maritime safety. Known for her methodical approach to harbor management and
close cooperation with Lord Mayor Marrick's maritime security initiatives.

## Career

### Harbor Authority

- Extensive experience in maritime operations
- Oversees daily port activities
- Coordinates with naval patrols
- Manages harbor infrastructure
- Supervises safety protocols

### Current Role

As Harbor Master since 1272, Navton has implemented:

- Enhanced vessel inspection procedures
- Modernized harbor navigation systems
- Improved emergency response protocols
- Streamlined docking schedules
- Updated safety regulations

## Notable Actions

- Renovation of harbor facilities
- Implementation of new safety measures
- Coordination of anti-piracy efforts
- Development of harbor patrol protocols
