...
Use `VehicleStatBlock` in MDX (and `.astro`) with ship data that matches `VehicleShipData` from
...
Parchment-style card, dark mode tokens, layout aligned with the site’s `StatBlock` presentation.
