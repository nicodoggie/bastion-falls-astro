...
export default function PanZoomViewer(props: Props) {
	const { src, width, height, alt, inlineHeight = "300px" } = props;

	// Use useMemo to generate stable IDs that won't change on re-renders
	const dialogId = useMemo(
		() => `panzoom-dialog-${Math.random().toString(36).slice(2)}`,
		[],
	);
	const fullscreenTargetId = useMemo(
		() => `panzoom-dialog-target-${Math.random().toString(36).slice(2)}`,
		[],
	);
	const inlineTargetId = useMemo(
		() => `panzoom-inline-${Math.random().toString(36).slice(2)}`,
		[],
	);
	const inlineModalBtnId = useMemo(
		() => `panzoom-inline-modal-btn-${Math.random().toString(36).slice(2)}`,
		[],
	);
	...
	}, [inlineModalBtnId, inlineTargetId]);

	const setDialogSize = useCallback((dlg: HTMLDialogElement) => {
	...
	const openDialog = useCallback(
		...
		[dialogId, fullscreenTargetId, setDialogSize, enterFullscreen],
	);
